...
function offlineTab() {
    ...
    var btnErase = {
        ...
        handler: function(btn){
			Ext.Msg.confirm('warning', 'Voulez vous enlever la référence de tous les documents à ce dossier?', function(r){
				if (r == 'yes') {
					eraseAllOnServer();
					var westGrid = Ext.getCmp('westGrid');
					var westGridM = Ext.getCmp('westGridModif');
    ...
    if (offlineModule){
    	var items_on_toolbar = [btnDl,btnOpenFolder,'->',btnmodify,btnRefresh,btnErase];
    }else{
    	 var items_on_toolbar = [btnOpenFolder,'->',btnmodify,btnRefresh,btnErase];
    }
	...
	var dragDropPanel = new Ext.Panel({
        ...
        html: "<div class='divDragDrop'>" +
        		"<p>glisser/déposer des documents.</p>" +
        		"</div>" 
		...
		listeners: {
			/*render: function(g) {
				g.on("beforetooltipshow", function(grid, row, col) {
					...
					if (col == 0 && grid.store.data.items[row].data.verrou !== "0" ){
						grid.tooltip.body.update("document verrouillé");
					}
					else if (col == 0 && grid.store.data.items[row].data.verrou == "0" ){
						grid.tooltip.body.update("document déverrouilé");
					}
					else if (col == 1){
    ...
    var gridModif = new Ext.grid.GridPanel({
		...
		listeners: {
			/*render: function(g) {
				g.on("beforetooltipshow", function(grid, row, col) {
					//grid.tooltip.body.update("Tooltip for (" + row + ", " + col + ")");
					if (col == 1 && grid.store.data.items[row].data.title != 'undefined'){
						grid.tooltip.body.update(grid.store.data.items[row].data.title);
					}
				});
			},*/
	...
	var downloadPanel = new Ext.Panel({
        ...
        title: generTitleDl(),
        html: "<div class='divFO'><ul>" +
	        	"<li class='liFO'>" +
	        		"<div class='iconDl' ></div>" +
	        		"<a href='"+ URL_INSTALL_WinApps +"' class='aFO'>" +
	        			"application pour Windows" +
	        		"</a>" +
	        	"</li>" +
	        	"<li class='liFO'>" +
	        		"<div class='iconDl' ></div>" +
	        		"<a href='"+ URL_INSTALL_LinApps +"' class='aFO'>" +
	        			"application pour Linux" +
	        		"</a>" +
	        	"</li>" +
	        	"<li class='liFO'>" +
	        		"<div class='iconDl' ></div>" +
	        		"<a href='"+ URL_INSTALL_XPI +"' class='aFO'>" +
	        			"extension Firefox" +
	        		"</a>" +
	        	"</li>" +
        	"<ul></div>" 
    ...
    var tab = new Ext.Panel({
		...
		listeners: {
			activate:function(tab){
				...
				//dragDrop
				var offlinedropTarget = new Ext.dd.DropTarget(Ext.getCmp('offlineTab').body, {
			        ddGroup: 'documentDD',
	...
	function title_img(val, x, store){
        return '<img src="' + store.data.icon + '" width="15" height="15" align="left">' +
...
function createDataGrid(docIdContainer){
    ...
    var d =  docIdContainer; 
	 if (d.isAlive()) {
	 	var p = d.getContent({completeProperties : true});
