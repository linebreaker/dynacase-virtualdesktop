...
ecm._documentListGadget={userPref:{collectionId:9},
			     hasParameters:false,
...
ecm.addGadget= function(gadget) {   
    ...
    if (! session.gadgets) session.gadgets=[];
    if (! cgadget.id) {
	var dt=new Date();
    ...
    var swin = new Ext.fdl.Service({gadget:cgadget});
    if (swin) swin.show();
    }
}
...
ecm.listGadgets=function () {
    var d=new Fdl.SearchDocument();
    var p=d.search({famid:'GADGET'});
    if (! p) Ext.Msg.alert(Fdl.getLastErrorMessage());
    else {
        ...
        for (var i = 0; i < p.length; i++) {
            var doc = p[i];
            data.push([doc.getProperty('id'), doc.getTitle(), doc.getIcon({ width: 32})]);
        }
	    ...
	    anchor:'100% 100%',
            ...
            columns: [{
                ...
                renderer: function render(value, p, record){
                    return String.format('<img src="{0}" style="float:left;height:24px;width:24px;" />', value);
                }
            }, {
                header: "Title",
                dataIndex: 'title',
		...
		afterlayout: function(o) {
		    ...
		    //this.ownerCt.setTitle(this.collection.getTitle());
                ...
                //                render: {
                //                    fn: function(grid){
                //                        this.view.dragZone = new GridDocumentDragZone(grid, {
                ...
                rowclick: function(grid, rowIndex, eventObject){
                    ...
                    var document = Fdl.ApplicationManager.getDocument(id,{reload:true});
                    ...
                    if (document.isAlive()) {
			var wg={extview:document.getValue('gad_viewwidget'),
				extparam:document.getValue('gad_editwidget'),
				width:parseInt(document.getValue('gad_width')),
				hasParameters:(document.getValue('gad_editwidget')!=null),
				height:parseInt(document.getValue('gad_height')),
				userPref:{}};
        ...
        // 'Hack' to remove the refresh button of the paging bar. Does not work...
